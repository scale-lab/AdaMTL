...
class MoEMlp(nn.Module):
    ...
    def _init_weights(self):
        if hasattr(self._moe_layer, "experts"):
            trunc_normal_(self._moe_layer.experts.batched_fc1_w,
                          std=self.init_std)
            trunc_normal_(self._moe_layer.experts.batched_fc2_w,
                          std=self.init_std)
    ...
    def flops(self):
        ...
        if self.is_moe:
            flops += 2 * H * W * self.dim * self.dim * \
                self.mlp_ratio * self.capacity_factor * self.top_value
        else:
