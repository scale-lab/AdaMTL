...
def norm_targets(targets, patch_size):
    ...
    targets_mean = F.avg_pool2d(targets, kernel_size=patch_size,
                                stride=1, padding=patch_size // 2, count_include_pad=False)
    ...
    targets_count = F.avg_pool2d(targets_count, kernel_size=patch_size, stride=1,
                                 padding=patch_size // 2, count_include_pad=True) * (patch_size ** 2)
...
def build_simmim(config):
    ...
    model = SimMIM(config=config.MODEL.SIMMIM, encoder=encoder,
                   encoder_stride=encoder_stride, in_chans=in_chans, patch_size=patch_size)
