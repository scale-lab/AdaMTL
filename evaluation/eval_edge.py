...
def eval_edge_predictions(p, database, save_dir):
    ...
    result_dir = os.path.join(
        seism_root, 'datasets/%s/%s/' % (database, exp_name))
    ...
    parameters_location = os.path.join(
        seism_root, 'parameters/%s.txt' % (exp_name))
    ...
    seism_base = os.path.join(
        PROJECT_ROOT_DIR, 'evaluation/seism/pr_curves_base.m')
    ...
    output_file += ['addpath(\'%s\')' %
                    (os.path.join(seism_root, 'src/scripts/'))]
    output_file += ['addpath(\'%s\')' %
                    (os.path.join(seism_root, 'src/misc/'))]
    output_file += ['addpath(\'%s\')' %
                    (os.path.join(seism_root, 'src/tests/'))]
    output_file += ['addpath(\'%s\')' %
                    (os.path.join(seism_root, 'src/gt_wrappers/'))]
    output_file += ['addpath(\'%s\')' % (os.path.join(seism_root, 'src/io/'))]
    output_file += ['addpath(\'%s\')' %
                    (os.path.join(seism_root, 'src/measures/'))]
    output_file += ['addpath(\'%s\')' %
                    (os.path.join(seism_root, 'src/piotr_edges/'))]
    output_file += ['addpath(\'%s\')' %
                    (os.path.join(seism_root, 'src/segbench/'))]
    ...
    cwd = os.getcwd()
    os.chdir(seism_root)
    os.system("matlab -nodisplay -nosplash -nodesktop -r \"addpath(\'%s\');%s;exit\"" %
              (seism_root, exp_name))
    os.chdir(cwd)
    ...
    result_rm = os.path.join(seism_root, 'results/%s/%s/' %
                             (database, exp_name))
    data_rm = os.path.join(seism_root, 'datasets/%s/%s/' %
                           (database, exp_name))
